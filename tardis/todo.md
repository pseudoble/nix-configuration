...
### 2. Cleanup PrismLauncher storage (74GB)
- Review ~/.local/share/PrismLauncher directory (74GB)
- Identify old/unused Minecraft modpacks and instances
- Remove unnecessary data, keep only active modpacks
- **Priority**: High for system health (root partition 97% full)

### 3. Fix home-manager warnings
- Address "Git tree is dirty" warning when running home-manager
- Fix deprecated `programs.zsh.initExtra` (use `programs.zsh.initContent` instead)
- Clean up any other warnings that appear during home-manager rebuild
- **Priority**: Medium - ensures clean builds and future compatibility

### 4. Diagnose system lockup when sunshine starts
- Look through bootup logs to identify issues that may contribute to system locking up when sunshine starts

### 5. Cleanup and organize nix configs SAFELY
...
### 9. Get Quest 2 VR headset working with Steam
